...
from tests.factories import (
    ActivityFactory,
    BuildingFactory,
    OrganizationActivityFactory,
    OrganizationFactory,
)

# Test constants
EXPECTED_ORGANIZATIONS_COUNT = 2
MIN_ORGANIZATIONS_COUNT = 2
...
async def test_get_organization_by_id(
    ...
    org = OrganizationFactory.build_with_building(building.id)

    db_session.add_all([building, org])
    await db_session.commit()

    response = await client.get(f'/organizations/organization/{org.id}/', headers=api_headers)
...
async def test_get_organization_not_found(
    ...
    response = await client.get(f'/organizations/organization/{fake_id}/', headers=api_headers)
...
async def test_search_organizations_by_name(
    ...
    building = BuildingFactory.build()
    org1 = OrganizationFactory.build_with_building(building.id, name='Apple Store')
    org2 = OrganizationFactory.build_with_building(building.id, name='Apple Inc')
    org3 = OrganizationFactory.build_with_building(building.id, name='Microsoft Store')

    db_session.add_all([building, org1, org2, org3])
    await db_session.commit()

    response = await client.get(
        '/organizations/search/name/', params={'name': 'Apple'}, headers=api_headers
    )
    assert response.status_code == status.HTTP_200_OK

    data = response.json()
    assert isinstance(data, list)
    # Should find organizations with "Apple" in the name
    apple_org_ids = {str(org1.id), str(org2.id)}
    returned_ids = {item['id'] for item in data}
    assert apple_org_ids == returned_ids


@pytest.mark.asyncio
async def test_search_organizations_by_query(
    client: AsyncClient, db_session: AsyncSession, api_headers: dict[str, str]
) -> None:
    """Test searching organizations by name or description."""
    building = BuildingFactory.build()
    org1 = OrganizationFactory.build_with_building(
        building.id, name='Horns and Hooves', description='Horn production'
    )
    org2 = OrganizationFactory.build_with_building(
        building.id, name='Another Company', description='Horn sales'
    )
    org3 = OrganizationFactory.build_with_building(
        building.id, name='Third Company', description='Furniture production'
    )

    db_session.add_all([building, org1, org2, org3])
    await db_session.commit()

    # Verify that organizations are saved
    await db_session.refresh(org1)
    await db_session.refresh(org2)
    await db_session.refresh(org3)

    response = await client.get(
        '/organizations/search/', params={'query': 'horns'}, headers=api_headers
    )
    assert response.status_code == status.HTTP_200_OK

    data = response.json()
    assert isinstance(data, list)
    # Should find organizations with "horns" in name or description
    returned_ids = {item['id'] for item in data}

    # Check that we get at least one organization with "horns"
    assert len(data) >= 1
    # Check that all returned organizations contain "horns" in name or description
    for item in data:
        assert 'horns' in item['name'].lower() or 'horns' in item['description'].lower()

    # Check that org1 is found (it has "horns" in the name)
    assert str(org1.id) in returned_ids


@pytest.mark.asyncio
async def test_get_organizations_by_building_alternative(
    client: AsyncClient, db_session: AsyncSession, api_headers: dict[str, str]
) -> None:
    """Test getting organizations by building (alternative endpoint)."""
    building = BuildingFactory.build()
...
async def test_get_organizations_by_activity(
    ...
    """Test getting organizations by activity type."""
    # Create activity and organizations
    activity = ActivityFactory.build_root()
    building = BuildingFactory.build()
    org1 = OrganizationFactory.build_with_building(building.id)
    org2 = OrganizationFactory.build_with_building(building.id)

    # First add activity, building and organizations
    db_session.add_all([activity, building, org1, org2])
    await db_session.commit()

    # Then create activity links
    org_activity1 = OrganizationActivityFactory.build_with_ids(org1.id, activity.id)
    org_activity2 = OrganizationActivityFactory.build_with_ids(org2.id, activity.id)

    db_session.add_all([org_activity1, org_activity2])
    await db_session.commit()

    response = await client.get(f'/organizations/activity/{activity.id}/', headers=api_headers)
    assert response.status_code == status.HTTP_200_OK
...
async def test_search_organizations_by_radius(
    ...
    far_building = BuildingFactory.build(latitude=55.7558 + 1.0, longitude=37.6176 + 1.0)

    # Create organizations in these buildings
    ...
    data = response.json()
    assert isinstance(data, list)
    # Should find organizations in Moscow, but not the distant one
    org_ids = {str(org1.id), str(org2.id)}
    returned_ids = {item['id'] for item in data}
    assert org_ids == returned_ids
